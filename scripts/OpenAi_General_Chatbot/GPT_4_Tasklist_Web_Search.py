...
def search_webscrape_db(line):
    ...
    try:
        with lock:
            ...
            results = vdb.query(vector=line_vec, filter={
        "memory_type": "web_scrape"}, top_k=30, namespace=f'{bot_name}_short_term_memory')
 ...
 #   r = sr.Recognizer()
    ...
    #    print(tasklist_output)
        ...
        "memory_type": "explicit_long_term"}, top_k=5, namespace=f'{bot_name}'),
                ...
                executor.submit(lambda: (
                    vdb.query(vector=vector_input, filter={
        "memory_type": "explicit_long_term"}, top_k=8, namespace=f'{bot_name}'),
                    load_conversation_explicit_short_term_memory)
                ),
                executor.submit(lambda: (
                    vdb.query(vector=vector_input, filter={
        "memory_type": "explicit_long_term"}, top_k=2, namespace=f'{bot_name}'),
       ...
       # # # Inner Monologue Generation
        ...
        "memory_type": "web_scrape"}, top_k=7, namespace=f'{bot_name}')
