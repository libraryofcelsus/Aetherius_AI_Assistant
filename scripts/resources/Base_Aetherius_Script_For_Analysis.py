...
def Base_Aetherius_Script_For_Analysis():
 ...
 #   r = sr.Recognizer()
    ...
    #    auto.append({'role': 'assistant', 'content': "I will now review the user's message and my reply, rating if whether my thoughts are both pertinent to the user's inquiry and my growth with a number on a scale of 1-10. I will now give my response in digit form for a python integer input: "})
    #    auto_int = None
    ...
    #        pass
        ...
        if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] % 2 == 0:
            ...
            consolidation.append({'role': 'system', 'content': "%s" % main_prompt})
            consolidation.append({'role': 'assistant', 'content': "LOG:\n%s\n\nRead the Log and consolidate the different topics into executive summaries. Each summary should contain the entire context of the memory. Follow the format [- Executive Summary]." % memory_consol_db2})
            memory_consol2 = chatgptsummary_completion(consolidation)
            consolidation.clear()
            print('Finished.\nRemoving Redundent Memories.')
            vector_sum = gpt3_embedding(memory_consol2)
            results = vdb.query(vector=vector_sum, top_k=15, namespace='implicit_long_term_memory')
            ...
            memory_consol3 = chatgptsummary_completion(consolidation)
        ...
        if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] > 3:
            ...
            consolidation.append({'role': 'system', 'content': "%s" % main_prompt})
            consolidation.append({'role': 'assistant', 'content': "LOG:\n%s\n\nRead the Log and consolidate the different memories into executive summaries in a process allegorical to associative processing. Each summary should contain the entire context of the memory. Follow the format [- Executive Summary]." % memory_consol_db1})
            memory_consol = chatgptsummary_completion(consolidation)
            print('\nOriginal Memories\n')
            print(memory_consol_db1)
            print('\nConsolidated Memories\n')
            print(memory_consol)
            print('\nWould you like to upload consolidated memories to DB?\nY for yes or N for no.')
            while True:
                user_input = input("'Y' or 'N': ")
                if user_input == 'y':
                    memories = results
