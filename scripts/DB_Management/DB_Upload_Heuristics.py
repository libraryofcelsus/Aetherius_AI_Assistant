    ...
    while True:
        ...
        if a == 'Delete All Data':
            while True:
                ...
                if user_input == 'y':
                    vdb.delete(filter={"memory_type": "heuristics"}, namespace=f'{bot_name}')
                    while True:
        ...
        metadata = {'speaker': 'AETHERIUS', 'time': timestamp, 'message': a, 'timestring': timestring,
                    'uuid': unique_id, "memory_type": "heuristics"}
        save_json('nexus/{bot_name}/{username}/heuristics_nexus/%s.json' % unique_id, metadata)
        payload.append((unique_id, vector, {"memory_type": "heuristics"}))
