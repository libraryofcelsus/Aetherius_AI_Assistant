...
def External_Resource_DB_Search(self, host, bot_name, username, line, task_counter, output_one, output_two, master_tasklist_output, a):
    try:
        ...
        if Search_Web == True:    
            websearch_check.append({'role': 'system', 'content': f"You are a selection agent for an autonomous AI chatbot.  Your job is to decide if the given database queries contain the needed information to answer the user's inquiry.  Only respond with either 'YES' or 'NO'."})
            websearch_check.append({'role': 'user', 'content': f"GIVEN DATABASE QUERIES: {table}\n\nUSER INQUIRY: {a}"})
            web_check = chatgptselector_completion(websearch_check)
            print(web_check)
            if "NO" in web_check:
                websearch_rephrase.append({'role': 'system', 'content': f"Extract any salient informational requests from the user's query and rephrase them into an informational search query that will return the requested information.  Only print the search query."})
                ...
                rephrased_query = chatgpt200_completion(websearch_rephrase)
                ...
                if Search_Engine == "Google":
                    try:
                        ...
                        if response.status_code == 200:
                            data = response.json()
                            table = [{'snippet': item['snippet'], 'url': item['link']} for item in data.get("items", [])]  # Return a list of snippets and URLs
                            table = f"Snippet Results from websearch:\n{table}"
                        else:
        ...
        conversation.append({'role': 'user', 'content': f"Summarize the pertinent information from the given external sources related to the given task. Present the summarized data in a single, easy-to-understand paragraph. Do not generalize, expand upon, or use any latent knowledge in your summary, only return a truncated version of previously given information."})
        conversation.append({'role': 'assistant', 'content': f"BOT {task_counter}: Sure, here's an overview of the scraped text: "})
        
        task_completion = chatgpt_summary_completion(conversation)
