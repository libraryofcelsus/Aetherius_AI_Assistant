...
class ChatBotApplication(tk.Frame):
    ...
    def delete_conversation_history(self):
        ...
        try:
            ...
            # Reload the script
            self.master.destroy()
            OpenAi_Local_Embed_Manual_Memory_Upload()
        except FileNotFoundError:
            pass
    ...
    def process_message(self, a):
        ...
        self.conversation_text.yview(tk.END)
        # Here, we're calling your Manual_Memory_Upload function in a separate thread
        t = threading.Thread(target=self.GPT_3_5_16_Inner_Monologue, args=(a,))
        t.start()


    def GPT_3_5_16_Inner_Monologue(self, a):
   ...
   #         a = input(f'\n\nUSER: ')
            ...
            t = threading.Thread(target=self.GPT_3_5_16_Intuition, args=(a, vector_input, output_one, int_conversation))
            t.start()
            return
    ...
    def GPT_3_5_16_Intuition(self, a, vector_input, output_one, int_conversation):
        vdb = pinecone.Index("aetherius")
        # # Number of Messages before conversation is summarized, higher number, higher api cost. Change to 3 when using GPT 3.5 due to token usage.
        m = multiprocessing.Manager()
        lock = m.Lock()
   ...
   #             pass   
            ...
            t = threading.Thread(target=self.GPT_3_5_16_Response, args=(a, output_one, output_two))
            t.start()
            return   
...
def OpenAi_Local_Embed_Manual_Memory_Upload():
    bot_name = open_file('./config/prompt_bot_name.txt')
    username = open_file('./config/prompt_username.txt')
    if not os.path.exists(f'nexus/{bot_name}/{username}/implicit_short_term_memory_nexus'):
        os.makedirs(f'nexus/{bot_name}/{username}/implicit_short_term_memory_nexus')
