...
def oobabooga_inner_monologue(prompt):
    ...
    request = {
        ...
        'mode': 'instruct',  # Valid options: 'chat', 'chat-instruct', 'instruct'
        'instruction_template': 'Vicuna-v1.1',  # Will get autodetected if unset
        'context_instruct': f"You are {bot_name}. Give a brief, first-person, silent soliloquy as your inner monologue that reflects on your contemplations in relation on how to respond to the user, {username}'s most recent message.",  # Optional
        'your_name': f'{username}',
...
class ChatBotApplication(tk.Frame):
    ...
    def choose_bot_name(self):
        ...
        if bot_name:
            ...
            self.display_conversation_history() 
            self.master.destroy()
            Qdrant_Experimental_Vicuna_Instruct_Chatbot_Manual_Memory_Upload()
...
def Qdrant_Experimental_Vicuna_Instruct_Chatbot_Manual_Memory_Upload():
    bot_name = open_file('./config/prompt_bot_name.txt')
    username = open_file('./config/prompt_username.txt')
    base_path = "./config/Chatbot_Prompts"
    base_prompts_path = os.path.join(base_path, "Base")
