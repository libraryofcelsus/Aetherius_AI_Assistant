        ...
        for chunk in chunks:
            ...
            for paragraph in paragraphs:  # Process each paragraph individually, add a check to see if paragraph contained actual information.
                ...
                if webyescheck == 'YES':
                    ...
                    save_json('nexus/role_extraction_nexus/%s.json' % unique_id, metadata)
                    payload.append((unique_id, vector))
                    vdb.upsert(payload, namespace='role_extractor')
                    payload.clear()
        table = weblist
        return table
    except Exception as e:
...
def load_conversation_role_extraction(results):
    try:
        result = list()
        for m in results['matches']:
            info = load_json('nexus/role_extraction_nexus/%s.json' % m['id'])
            result.append(info)
        ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
        messages = [i['message'] for i in ordered]
        return '\n'.join(messages).strip()
...
def process_line(line, vdb, extractor_questions, gpt3_embedding, chatgptresponse_completion, save_json, timestamp_to_datetime, bot_name, user_selection):
    ...
    results = vdb.query(vector=line_vec, top_k=22, namespace='role_extractor')
...
def GPT_4_Role_Extractor():
 ...
 #   r = sr.Recognizer()
    ...
    #    process_files_in_directory('./Upload/EPUB', './Upload/EPUB/Finished')
        ...
        results = vdb.query(vector=line_vec, filter={"memory_type": "implicit_long_term"}, top_k=35, namespace=f'{user_selection}')
        question_db1 = load_conversation_implicit_long_term_memory(results)
        
        results = vdb.query(vector=line_vec, filter={"memory_type": "explicit_long_term"}, top_k=35, namespace=f'{user_selection}')
