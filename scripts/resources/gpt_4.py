...
def chatgptyesno_completion(messages, model="gpt-3.5-turbo", temp=0.1):
    ...
    while True:
        try:
            response = openai.ChatCompletion.create(model=model, messages=messages, max_tokens=0, top_p=0.1)
            text = response['choices'][0]['message']['content']
            temperature = temp
...
def chatgptselector_completion(messages, model="gpt-3.5-turbo", temp=0.1):
    ...
    with lock:
        ...
        #    save_file('chat_logs/%s' % filename, str(messages) + '\n\n==========\n\n' + text)
            ...
            while True:
                try:
                    response = openai.ChatCompletion.create(model="gpt-4", messages=messages, max_tokens=4)
                    text = response['choices'][0]['message']['content']
                    temperature = temp
                    return text
                except Exception as oops:
...
def chatgptauto_completion(messages, model="gpt-4", temp=0.35):
    max_retry = 7
    retry = 0
    while  True:
        try:
...
def chatgptconsolidation_completion(messages, model="gpt-3.5-turbo", temp=0.1):
    ...
    while True:
        try:
            response = openai.ChatCompletion.create(model=model, messages=messages, max_tokens=600)
            text = response['choices'][0]['message']['content']
            temperature = temp
            return text
        except Exception as oops:
