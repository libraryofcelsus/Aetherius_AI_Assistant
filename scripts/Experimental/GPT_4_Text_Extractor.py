...
def search_implicit_db(line_vec):
    ...
    try:
        with lock:
            print('implicit')
            results = vdb.query(vector=line_vec, filter={"memory_type": "implicit_long_term"}, top_k=7, namespace=f'{bot_name}')
            memories1 = load_conversation_implicit_long_term_memory(results)
            results = vdb.query(vector=line_vec, filter={"memory_type": "implicit_short_term"}, top_k=5, namespace=f'{bot_name}_short_term_memory')
...
def search_episodic_db(line_vec):
    ...
    try:
        with lock:
            ...
            "memory_type": "episodic"}, top_k=5, namespace=f'{bot_name}')
...
def search_flashbulb_db(line_vec):
    ...
    try:
        with lock:
            results = vdb.query(vector=line_vec, filter={
            "memory_type": "flashbulb"}, top_k=5, namespace=f'{bot_name}')
...
def search_explicit_db(line_vec):
    ...
    try:
        with lock:
            print('explicit')
            results = vdb.query(vector=line_vec, filter={"memory_type": "explicit_long_term"}, top_k=5, namespace=f'{bot_name}')
            memories1 = load_conversation_explicit_long_term_memory(results)
            results = vdb.query(vector=line_vec, filter={"memory_type": "explicit_short_term"}, top_k=5, namespace=f'{bot_name}_short_term_memory')
        ...
        for chunk in chunks:
            ...
            for paragraph in paragraphs:  # Process each paragraph individually, add a check to see if paragraph contained actual information.
                ...
                if fileyescheck == 'YES':
                    ...
                    vdb.upsert(payload, namespace=f'{bot_name}_short_term_memory')
                    payload.clear()
                    filecheck.clear()
        table = filelist
        return table
...
def search_file_process_db(line):
    ...
    try:
        with lock:
            ...
            results = vdb.query(vector=line_vec, filter={
        "memory_type": "file_process"}, top_k=30, namespace=f'{bot_name}_short_term_memory')
...
def GPT_4_Text_Extractor():
 ...
 #   r = sr.Recognizer()
        ...
        if a == 'Clear Memory':
            while True:
                ...
                if user_input == 'y':
                    ...
                    vdb.delete(filter={"memory_type": "file_process"}, namespace=f'{bot_name}_short_term_memory')
                    while True:
    ...
    #    print(tasklist_output)
        ...
        "memory_type": "explicit_long_term"}, top_k=3, namespace=f'{bot_name}'),
                        db_term_result.update({_index: load_conversation_explicit_long_term_memory(results)}),
                        results := vdb.query(vector=db_term[_index], filter={
        "memory_type": "implicit_long_term"}, top_k=2, namespace=f'{bot_name}'),
        ...
        "memory_type": "explicit_short_term"}, top_k=8, namespace=f'{bot_name}'),
                ...
                executor.submit(lambda: (
                    vdb.query(vector=vector_input, filter={
        "memory_type": "flashbulb"}, top_k=2, namespace=f'{bot_name}'),
                    load_conversation_flashbulb_memory)
                ),
                executor.submit(lambda: (
                    vdb.query(vector=vector_input, filter={
        "memory_type": "explicit_long_term"}, top_k=5, namespace=f'{bot_name}'),
       ...
       # # # Inner Monologue Generation
        ...
        "memory_type": "explicit_short_term"}, top_k=10, namespace=f'{bot_name}_short_term_memory')
        ...
        "memory_type": "heuristics"}, top_k=3, namespace=f'{bot_name}')
            try:
        ...
        "memory_type": "file_process"}, top_k=7, namespace=f'{bot_name}short_term_memory')
    ...
    #        pass                 
        ...
        namespace_name = f'{bot_name}_short_term_memory'
        if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] > 25:
            ...
            results = vdb.query(vector=vector_input, filter={
        "memory_type": "explicit_short_term"}, top_k=25, namespace=f'{bot_name}_short_term_memory')
            ...
            results = vdb.query(vector=vector_input, filter={
        "memory_type": "implicit_short_term"}, top_k=20, namespace=f'{bot_name}_short_term_memory')
        ...
        "memory_type": "implicit_long_term"}, top_k=8, namespace=f'{bot_name}')
            ...
            if user_input == 'y':
                ...
                for line in lines:
                    if line.strip():
                        ...
                        vdb.upsert(payload, namespace=f'{bot_name}')
                        payload.clear()
                vdb.delete(delete_all=True, namespace=f'{bot_name}_short_term_memory')
        ...
        "memory_type": "implicit_long_term"}, top_k=10, namespace=f'{bot_name}')
        ...
        "memory_type": "implicit_long_term"}, namespace=f'{bot_name}')
                        break
                    elif user_input == 'n':
                        print('Cancelled')
                        break
        ...
        # # Explicit Long-Term Memory Associative Processing/Pruning based on amount of vectors in namespace
            ...
            if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] > 2:
                ...
                results = vdb.query(vector=vector_monologue, filter={
        "memory_type": "implicit_long_term"}, top_k=5, namespace=f'{bot_name}')
        ...
        "memory_type": "explicit_long_term"}, top_k=10, namespace=f'{bot_name}')
