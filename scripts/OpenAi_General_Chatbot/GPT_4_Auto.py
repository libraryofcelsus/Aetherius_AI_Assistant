 ...
 #   r = sr.Recognizer()
    ...
    #    a = (f'\n\nUSER: {text}')
        ...
        "memory_type": "episodic"}, top_k=5, namespace=f'{bot_name}'),
                ...
                executor.submit(lambda: (
                    vdb.query(vector=vector_input, filter={
        "memory_type": "explicit_short_term"}, top_k=8, namespace=f'{bot_name}_short_term_memory'),
    ...
    #        print(db_search_6)
        ...
        "memory_type": "implicit_long_term"}, top_k=5, namespace=f'{bot_name}')
            future2 = executor.submit(vdb.query, vector=vector_input, filter={
        "memory_type": "episodic"}, top_k=10, namespace=f'{bot_name}')
            future3 = executor.submit(vdb.query, vector=vector_monologue, filter={
        "memory_type": "flashbulb"}, top_k=2, namespace=f'{bot_name}')
            try:
    ...
    #        print(conv_summary)
        ...
        if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] > 30:
            ...
            results = vdb.query(vector=vector_input, filter={"memory_type": "explicit_short_term"}, top_k=25, namespace=f'{bot_name}_short_term_memory')
            ...
            #    print(timestring + line)
                    ...
                    vdb.upsert(payload, namespace=f'{bot_name}')
                    payload.clear()
            vdb.delete(filter={"memory_type": "explicit_short_term"}, namespace=f'{bot_name}_short_term_memory')
        ...
        # # Implicit Short Term Memory Consolidation based on amount of vectors in namespace
            ...
            if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] % 2 == 0:
                ...
                results = vdb.query(vector=vector_input, filter={"memory_type": "implicit_short_term"}, top_k=20, namespace=f'{bot_name}_short_term_memory')
                ...
                results = vdb.query(vector=vector_sum, filter={"memory_type": "implicit_long_term"}, top_k=8, namespace=f'{bot_name}')
                ...
                for line in lines:
                    if line.strip():
                        ...
                        vdb.upsert(payload, namespace=f'{bot_name}')
                        payload.clear()
                vdb.delete(filter={"memory_type": "implicit_short_term"}, namespace=f'{bot_name}_short_term_memory')
                print('Memory Consolidation Successful')
            else:
                pass
        # # Implicit Associative Processing/Pruning based on amount of vectors in namespace
            ...
            if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] % 4 == 0:
                ...
                results = vdb.query(vector=vector_monologue, filter={"memory_type": "implicit_long_term"}, top_k=10, namespace=f'{bot_name}')
        ...
        # # Explicit Long-Term Memory Associative Processing/Pruning based on amount of vectors in namespace
            ...
            if namespace_name in namespace_stats and namespace_stats[namespace_name]['vector_count'] > 5:
                ...
                results = vdb.query(vector=vector_monologue, filter={"memory_type": "implicit_long_term"}, top_k=5, namespace=f'{bot_name}')
                ...
                results = vdb.query(vector=consol_vector, filter={"memory_type": "explicit_long_term"}, top_k=10, namespace=f'{bot_name}')
