...
def Memory_DB_Search(self, host, bot_name, username, line, task_counter, output_one, output_two, master_tasklist_output, a):
    try:
        ...
        try:
            conversation.append({'role': 'system', 'content': f"MAIN SYSTEM PROMPT: You are a sub-agent for {bot_name}, an Autonomous Ai-Chatbot. You are one of many agents in a chain. You are to take the given task and complete it in its entirety, using the given memories to ensure factual accuracy. Be Verbose and take other tasks into account when formulating your answer."})
            ...
            memcheck.append({'role': 'user', 'content': f"USER INPUT: {line}\nYou may only print Yes or No. Use the format: 'YES OR NO'"})
            memcheck.append({'role': 'assistant', 'content': f"YES/NO: "})
                # # DB Selector Tool
            ...
            memcheck2.append({'role': 'user', 'content': "//BEGIN JOB:\n"})
            ...
            try:
                mem1 = chatgptselector_completion(memcheck)
            except Exception as e:
                print(e)
            try:
                # mem1 := chatgptyesno_completion(memcheck)
                # # Go to conditional for choosing DB Name
                mem2 = chatgptselector_completion(memcheck2) if 'YES' in mem1.upper() else fail()
                ...
                print(e)
            conversation.append({'role': 'user', 'content': f"Summarize the pertinent information from the given memories related to the given task. Present the summarized data in a single, easy-to-understand paragraph. Do not generalize, expand upon, or use any latent knowledge in your summary, only return a truncated version of previously given information."})
