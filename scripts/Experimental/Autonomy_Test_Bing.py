...
def google_search1(line, my_api_key, my_cse_id, **kwargs):
...
def google_search(line, my_api_key, my_cse_id, **kwargs):
  ...
  if response.status_code == 200:
    ...
    html_table += "<table>"
    # Add the opening tag for the table header row
    html_table += "<tr>"
    # Add the table header cells for link and snippet
    html_table += "<th>Link</th>"
    html_table += "<th>Snippet</th>"
    ...
    for item in data.get("items", []):
      ...
      html_table += f"<td><a href='{item['link']}'>{item['link']}</a></td>"
