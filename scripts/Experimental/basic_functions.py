...
def load_conversation_explicit_short_term_memory(results):
    ...
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/explicit_short_term_memory_nexus/%s.json' % m['id'])
        result.append(info)
...
def load_conversation_explicit_long_term_memory(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/explicit_long_term_memory_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip()
...
def load_conversation_episodic_memory(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/episodic_memory_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip()  
...
def load_conversation_flashbulb_memory(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/flashbulb_memory_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip() 
...
def load_conversation_heuristics(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/heuristics_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip()    
...
def load_conversation_implicit_short_term_memory(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/implicit_short_term_memory_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip()  
...
def load_conversation_cadence(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/cadence_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip() 
...
def load_conversation_implicit_long_term_memory(results):
    ...
    result = list()
    for m in results['matches']:
        info = load_json('nexus/{bot_name}/implicit_long_term_memory_nexus/%s.json' % m['id'])
        result.append(info)
    ordered = sorted(result, key=lambda d: d['time'], reverse=False)  # sort them all chronologically
    messages = [i['message'] for i in ordered]
    return '\n'.join(messages).strip()
